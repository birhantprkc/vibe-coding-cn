...
| `operators/` | Reference Profile、417 个原始方法、60 个派生 Method、来源清单、catalog 与双轴 taxonomy | 不执行方法，不把本地 Profile 冒充 Core | `validate_harness.py --operator-library operators/catalog.json` |
| `skills/solve/` | 面向 AI 的自包含 Skill 说明与 477 条目参考快照 | 不编辑内容真相源，不执行工具、权限或业务状态 | `validate-skill.sh --strict` 与包内 operator-library 校验 |
...
`research/` 固定官方上游输入，`docs/` 和 ADR 定义语义，`contracts/` 固化结构，`operators/` 保存静态参考内容，`skills/solve/` 发布面向 AI 的可移植快照，`scripts/` 消费契约并输出准入结果，
